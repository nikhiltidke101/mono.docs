...
interface Components {
  ...
  OpenAPIFolder: React.FC<{ item: PageTree.OpenAPIFolder; level: number }>;
}
...
function APIFolderNode({
  item: { name, children, defaultOpen = false },
  level,
}: {
  ...
  const shouldExtend = childActive || defaultOpenLevel >= level || defaultOpen;
  const [extend, setExtend] = useState(shouldExtend);
